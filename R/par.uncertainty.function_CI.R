...
par.uncertainty_ci = function(vario.mod.output, mod.nr,
                           par.est = NULL, data = NULL, max.dist = NULL, nbins = NULL,
                           B = 1000, threshold.factor = 1.2, alpha = NULL, fit.method = 7, mc.cores = 1){
  ...
  alpha.arg <- deparse(substitute(alpha))
  fit.method.arg <- deparse(substitute(fit.method))
  mc.cores.arg <- deparse(substitute(mc.cores))
  ...
  if(is.null(alpha)){
    ...
    nr_estimates = sum(apply(par.est.b[,-(1:3)], 1, sum) == 0)
    ...
    for(i in 1:nr.thr){
      par.est.cleaned = par.est.b[which(par.est.b[,4] == 0 & par.est.b[,4+i] == 0),][1:B,]
    ...
    par.est.b = t(parallel::mclapply(rep(0, B), one_resample_analysis_check_alpha, y.iid=y.iid, L=L,
                                     nscore.obj = nscore.obj, coords = coords,
                                     max.dist = max.dist, nbins = nbins,
                                     threshold.factor=threshold.factor,
                                     alpha = alpha,
    ...
    colnames(par.est.b) = c("nugget", "partial_sill", "range", "conv_filter", "CI_filter",paste0("thr_filter_", threshold.factor))

    nr_estimates = sum(apply(par.est.b[,4:5], 1, sum) == 0)
    ...
    while(nr_estimates < B){
      if(B-nr_estimates < 20) mc.cores = 1
      re.par.est = t(parallel::mclapply(rep(0, B-nr_estimates), one_resample_analysis_check_alpha, y.iid=y.iid, L=L,
                                        nscore.obj = nscore.obj, coords = coords,
                                        max.dist = max.dist, nbins = nbins,
                                        threshold = threshold.factor,
                                        alpha = alpha,
    ...
    par.est.cleaned = par.est.b[which(par.est.b[,4] == 0 & par.est.b[,5] == 0),][1:B,]
    par.sds = apply(par.est.cleaned[,1:3], 2, stats::sd)
  }

  par.re_est = par.est.b[which(apply(par.est.b[,-(1:3)], 1, sum) == 0),1:3] # only those who passed all filters
  colnames(par.re_est) = c("nugget.star","part.sill.star","shape.star")
  reest.means = colMeans(par.re_est)
  ...
  if(is.null(alpha)){ # threshold option
    ...
    unc.table = as.data.frame(cbind(par.est, par.sds))
    colnames(unc.table) = c("Estimate", "Std. Error")

    nr_reest_gstat = sum(par.est.b[,4])
    nr_reest_alpha = sum(par.est.b[,5])
    nr_overlap = sum(par.est.b[,4]==1 & par.est.b[,5]==1)

    nr_reest_table = matrix(cbind(nr_reest_gstat, nr_reest_alpha, nr_overlap))
    rownames(nr_reest_table) = c("nr_reest_gstat", "nr_reest_alpha", "nr_overlap")
    colnames(nr_reest_table) = paste("alpha =", alpha)
  }
