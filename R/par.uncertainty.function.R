...
par.uncertainty = function(vario.mod.output, mod.nr,
  ...
  # calling the par.uncertainty.thr function in all cases.

  # 1. Case: Without threshold.factor. Default value of 1.2 is used
  if(is.null(threshold.factor)){
    par.uncertainty.thr(vario.mod.output = vario.mod.output, mod.nr = mod.nr,
                        par.est = par.est, data = data, max.dist = max.dist, nbins = nbins,
                        B = B)
  }

  # 2. Case: With threshold.factor as specified in the function call.
  else{
    par.uncertainty.thr(vario.mod.output = vario.mod.output, mod.nr = mod.nr,
                        par.est = par.est, data = data, max.dist = max.dist, nbins = nbins,
                        B = B, threshold.factor = threshold.factor)
  }
}
