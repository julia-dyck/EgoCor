...
v <- vgm(psill = var(log(meuse$zinc)),
          model = "Exp",
          range = 1500/3,
          nugget = 0)
...
sse1
# compare to
sse1.fit = attr(vfit,"SSErr") # sum of squared errors acc. to gstat output

##################
# fit.method = 2 # HIER NOCHMAL GUCKEN!
##################
vfit = fit.variogram(variog, model=v,  # fitting the model with starting model
                     fit.sills = TRUE,
                     fit.ranges = TRUE,
...
# compare to
sse2.fit = attr(vfit,"SSErr") # sum of squared errors acc. to gstat output
...
# compare to
sse6.fit = attr(vfit,"SSErr") # sum of squared errors acc. to gstat output
...
# compare to
sse7.fit = attr(vfit,"SSErr") # sum of squared errors acc. to gstat output
